...
const PHASE_TEMPLATES = {
  ...
  new_feature: [
    ...
    { phase: 'implement_backend',  role: 'implementer', order: 3, dependsOn: ['plan'] },
    { phase: 'implement_frontend', role: 'implementer', order: 3, dependsOn: ['plan'] },
    { phase: 'review',             role: 'reviewer',    order: 4, dependsOn: ['implement_backend', 'implement_frontend'] },
  ],
  refactor: [
    { phase: 'analyze',       role: 'analyst',     order: 1 },
    { phase: 'plan',          role: 'planner',     order: 2, dependsOn: ['analyze'] },
    { phase: 'execute_core',  role: 'implementer', order: 3, dependsOn: ['plan'] },
    { phase: 'execute_tests', role: 'implementer', order: 3, dependsOn: ['plan'] },
    { phase: 'verify',        role: 'reviewer',    order: 4, dependsOn: ['execute_core', 'execute_tests'] },
  ],
  optimize: [
  ...
  explore: [
    { phase: 'gather',     role: 'researcher', order: 1 },
    { phase: 'synthesize', role: 'analyst',    order: 2, dependsOn: ['gather'] },
    { phase: 'report',     role: 'consultant', order: 3, dependsOn: ['synthesize'] },
  ],
  analyze: [
    { phase: 'collect',        role: 'researcher',  order: 1 },
    { phase: 'evaluate_quant', role: 'analyst',     order: 2, dependsOn: ['collect'] },
    { phase: 'evaluate_qual',  role: 'analyst',     order: 2, dependsOn: ['collect'] },
    { phase: 'conclude',       role: 'consultant',  order: 3, dependsOn: ['evaluate_quant', 'evaluate_qual'] },
  ],
  content: [
    { phase: 'research_facts', role: 'researcher',  order: 1 },
    { phase: 'research_style', role: 'researcher',  order: 1 },
    { phase: 'draft',          role: 'implementer', order: 2, dependsOn: ['research_facts', 'research_style'] },
    { phase: 'review',         role: 'reviewer',    order: 3, dependsOn: ['draft'] },
  ],
  question: [
