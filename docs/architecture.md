...
shared type definitions. No business logic resides here. V5.1 adds MonotonicClock for accurate timing.

基础层。提供数据库访问、配置、模式定义和共享类型。此层不含业务逻辑。V5.1 新增 MonotonicClock 用于精确计时。
...
| `monotonic-clock.js` | V5.1: `process.hrtime.bigint()` monotonic clock for accurate duration measurement |
...
```
5 built-in bee personas (V5.1: +designer-bee):
  ┌──────────────────┬──────────────────────────────────────────────────────┐
  │ Persona          │ Best For                                             │
  ├──────────────────┼──────────────────────────────────────────────────────┤
  ...
  │ queen-messenger  │ plan, architecture, coordinate, strategy             │
  │ designer-bee     │ visualization, dashboard, design, chart, UI, layout  │
  └──────────────────┴──────────────────────────────────────────────────────┘
...
zone management. This layer decides *who does what, when, and how well*. V5.1 adds hierarchical swarm coordination, advanced DAG orchestration, and species evolution.
...
Responsibilities:
  ...
  getHooks()  → returns 14 hook handler functions (V5.1: 8 new hooks)
                (gateway_start, before_model_resolve, before_tool_call,
                 before_prompt_build, before_agent_start, agent_end,
                 after_tool_call, before_reset, gateway_stop,
                 message_sending, subagent_spawning, subagent_spawned,
                 subagent_ended, llm_output)
