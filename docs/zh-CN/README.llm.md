...
OpenClaw 插件（V9.0.0），使用仿生算法协调多个 LLM 代理。在 OpenClaw Gateway 进程内运行。场中介耦合架构：7 个自治域通过 12 维连续信号场通信。
...
121 个源文件，7 域架构。12 维信号场。27 个事件主题。16 个钩子。10 个工具。35+ 内置模型画像。57+ REST 端点（端口 19100）。
