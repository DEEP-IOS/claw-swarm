...
> Claw-Swarm V9.2.0 — 仿生群体智能插件（OpenClaw 生态）
...
V9.2 采用 **单进程网关内** 架构。所有 7 个域（约 121 个源文件）在 OpenClaw Gateway 进程内运行。无子进程 fork，无 IPC 桥接，无工作线程池。
...
| `dashboard.port` | `19100` | HTTP Dashboard + Console SPA 端口 |
...
安装器支持多种补丁模式，控制 Claw-Swarm 如何集成到 OpenClaw Gateway。

```bash
node install.js                          # 默认: --patch-mode=both
node install.js --patch-mode=loader      # 仅 loader hook
node install.js --patch-mode=patcher     # 仅 patcher
node install.js --patch-mode=both        # patcher + loader hook（默认）
node install.js --patch-mode=none        # 仅注册插件，不做任何补丁
```

| 模式 | 描述 |
|------|------|
| `loader` | 仅在 Gateway 运行时安装 loader hook |
| `patcher` | 仅对 Gateway 源文件应用静态 patcher |
| `both` | 同时应用 patcher 和 loader hook（默认，推荐） |
| `none` | 仅注册插件条目，不修改 Gateway |

### 构建控制台

控制台 SPA 在 npm 包中已预构建。如果是 git 克隆或需要重新构建：
...
构建产物放置在 `src/observe/dashboard/console/`，由 DashboardService 自动提供服务。

---

## 12 维信号场

信号场是 V9.2 架构的基础底层。它提供响应式通信层，所有 7 个域都依赖于此。
...
Claw-Swarm V9.2 使用 DomainStore + JSON 快照进行持久化，替代 V8 的 SQLite 数据库。
...
V9.2 采用**零特性标志**架构。所有约 110 个模块（横跨 7 个域）始终激活。模块激活完全通过 `ModuleBase` 的 produces/consumes 耦合机制控制：声明了 `consumes: ['signal_type']` 的模块在该类型信号存入信号场时自动激活。无需手动标志配置。
...
```bash
openclaw gateway status                        # 应显示: claw-swarm 9.2.0 enabled
curl http://127.0.0.1:19100/api/metrics        # 应返回 RED 指标 JSON
curl http://127.0.0.1:19100/api/v9/bridge/status
```

然后在浏览器中打开 `http://127.0.0.1:19100/v9/console`，确认 V9 控制台加载 10 个视图，并确认浏览器已经建立到 `19101` 的 WebSocket 连接。旧的 SSE 流仍然可用于调试，但当前 V9 控制台以 WebSocket bridge 为主。
