...
describe('Install V7.0 — install.js file checks', () => {
  it('install.js should reference V7.0', () => {
    const content = readFileSync(join(ROOT, 'install.js'), 'utf-8');
    expect(content).toContain('V7.0');
  });
  ...
  it('install.js should include v70FullLanding flags', () => {
    const content = readFileSync(join(ROOT, 'install.js'), 'utf-8');
    expect(content).toContain('v70FullLanding');
    expect(content).toContain('communicationSensing');
    expect(content).toContain('budgetDegradation');
  });
  ...
  it('swarm-run-tool.js mapRoleToOpenClawAgent should accept agentMapping', () => {
    const content = readFileSync(join(ROOT, 'src/L5-application/tools/swarm-run-tool.js'), 'utf-8');
    expect(content).toContain('mapRoleToOpenClawAgent(roleName, agentMapping)');
  });

  it('swarm-core.js should have agentMapping propagation', () => {
    const content = readFileSync(join(ROOT, 'src/swarm-core.js'), 'utf-8');
    expect(content).toContain('this._agentMapping');
    expect(content).toContain('engines.agentMapping');
  });

  it('swarm-core.js should have warm start logic', () => {
    const content = readFileSync(join(ROOT, 'src/swarm-core.js'), 'utf-8');
    expect(content).toContain('warmStart');
    expect(content).toContain('Warm start');
  });

  it('swarm-core.js _loadSoulFiles should support dynamic mapping', () => {
    const content = readFileSync(join(ROOT, 'src/swarm-core.js'), 'utf-8');
    expect(content).toContain('this._agentMapping');
