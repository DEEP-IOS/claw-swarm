...
# Claw-Swarm V5.2 --- 详细中文文档

**6 层仿生蜂群智能插件 | OpenClaw 多智能体协作基础设施**

> Node.js >= 22.0.0 | 75+ 源文件 | 659 测试 | 18+ 仿生算法 | 7 工具 | 14 钩子 | 5 蜜蜂人格 | 44 张数据库表
...
```
┌─────────────────────────────────────────────────────────────────┐
│ L6 监控层 (6)   StateBroadcaster(SSE) MetricsCollector(RED)     │
│   HealthChecker(空闲检测) DashboardService(Jaeger-lite追踪)      │
├─────────────────────────────────────────────────────────────────┤
│ L5 应用层 (13)  PluginAdapter  ContextService  CircuitBreaker   │
│   ToolResilience(修复记忆) SkillGovernor  TokenBudgetTracker    │
│   swarm_spawn/query/pheromone/gate/memory/plan/zone             │
├─────────────────────────────────────────────────────────────────┤
│ L4 编排层 (15)  Orchestrator  CriticalPath  QualityCtrl         │
...
│   SpeciesEvolver(LV种群动力学 + ABC三阶段进化)                    │
├─────────────────────────────────────────────────────────────────┤
│ L3 智能体层 (12) WorkingMemory  EpisodicMemory  SemanticMemory  │
...
│   SkillSymbiosisTracker(余弦互补)                                │
├─────────────────────────────────────────────────────────────────┤
│ L2 通信层 (6)   MessageBus  PheromoneEngine  GossipProtocol     │
│   PheromoneTypeRegistry  PheromoneResponseMatrix(压力梯度)       │
│   StigmergicBoard(持久公告板)                                    │
├─────────────────────────────────────────────────────────────────┤
...
V5.2 通过 OpenClaw Plugin SDK 的 `{ id, register(api) }` 模式注册。

### OpenClaw 事件 -> V5.2 映射

通过 Plugin SDK 注册 14 个钩子（V5.0: 6 + V5.1: 8 新增）：

| OpenClaw 事件 | V5.2 内部处理器 | 执行动作 |
|---------------|-----------------|----------|
...
| 层级 | 类型 | 数量 | 覆盖范围 |
|------|------|------|----------|
| 单元测试 | vitest | 659 tests / 43 files | 6 层全覆盖，每个模块独立验证 |
...
```bash
npm test                    # 全部 (659 tests, 43 files)
