...
# Claw-Swarm V5.1 --- 详细中文文档

**6 层仿生蜂群智能插件 | OpenClaw 多智能体协作基础设施**

> Node.js >= 22.0.0 | 66+ 源文件 | 573 测试 | 12+ 仿生算法 | 7 工具 | 14 钩子 | 5 蜜蜂人格
...
V5.1 通过 OpenClaw Plugin SDK 的 `{ id, register(api) }` 模式注册。

### OpenClaw 事件 -> V5.1 映射

通过 Plugin SDK 注册 14 个钩子（V5.0: 6 + V5.1: 8 新增）：

| OpenClaw 事件 | V5.1 内部处理器 | 执行动作 |
|---------------|-----------------|----------|
...
| 层级 | 类型 | 数量 | 覆盖范围 |
|------|------|------|----------|
| 单元测试 | vitest | 573 tests / 30+ files | 6 层全覆盖，每个模块独立验证 |
...
```bash
npm test                    # 全部 (573 tests, 30+ files)
...
tests/unit/L1~L6/ + integration/ + stress/    # 573 tests across 30+ files
```
