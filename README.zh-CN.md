...
| 等级 | 模型 | 说明 |
|---|---|---|
| **S 强烈推荐** | Claude Opus 4, Claude Sonnet 4.5, GPT-4.1, GPT-4o, Gemini 2.5 Pro | 工具调用 + 推理最强 |
| **A 推荐** | Kimi K2.5, Qwen3.5-Plus/Max, DeepSeek-V3, Gemini 2.5 Flash, Claude Haiku 4.5 | 能力强，有少量取舍 |
| **B 可用** | DeepSeek-R1, GLM-5, Qwen3-Coder, Mistral Large, Llama 4 Maverick | 适合特定角色 |

**经济方案（中文优先）**: 主 Agent 用 Kimi K2.5，子 Agent 用 Qwen3.5-Plus（百炼免费）

**均衡方案**: 主 Agent 用 Claude Sonnet 4.5，子 Agent 用 GPT-4o 或 Qwen3.5-Plus

**混合策略（推荐）**: 主 Agent 用强模型协调，子 Agent 用便宜模型执行
